...
/* TODO: will be implemented later for SBUS protocol */
...
static void uartReconfigure(uartPort_t *uartPort)
{
	...
	/* Configure the word length to 8 bits or 9 bits
	...
	/* Initialise USART_InitStructure data type */
	USART_Init(uartPort->USARTx, &USART_InitStructure);
	
	/* Configure USART pin inversion */
...
{
	uint32_t rxDMAHead;
	const uartPort_t *s = (const uartPort_t *)instance;		// since serialPort_t is the first element of the uartPort_t
#ifdef STM32F4
	if (s->rxDMAStream) {
		rxDMAHead = s->rxDMAStream->NDTR;
